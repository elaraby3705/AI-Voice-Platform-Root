...
import { useAuth } from '../context/AuthContext'; // Ensure this path matches your context location
import { Link } from 'react-router-dom';
import { Mail, Lock, Sparkles, ArrowRight, Loader2, ShieldCheck } from 'lucide-react';

const Register = () => {
    const { register } = useAuth(); // Connect to the "Brain"
    ...
    const handleSubmit = async (e) => {
        ...
        // We pass the object expected by your backend
        ...
        if (!success) {
            ...
            // Error handling is usually done inside register/toast,
            // but you can set specific error states here if needed.
        }
        // If success, the AuthContext usually redirects, so no need to set false here immediately
    };

    return (
        // Changed to min-h-screen to ensure perfect vertical centering
        <div className="flex flex-col items-center justify-center min-h-screen px-4 bg-[#050505]">
            ...
            <div className="w-full max-w-md relative z-10">
                ...
                <div className="text-center mb-8">
                    ...
                    <p className="text-slate-400 mt-2 text-sm">Join the neural network and start generating.</p>
                </div>

                {/* 📝 Form Card */}
                <div className="bg-black/40 backdrop-blur-xl border border-white/10 p-8 rounded-2xl shadow-2xl">

                    {/* Error Banner (Only shows if password mismatch) */}
                    {error && (
                        <div className="mb-6 p-3 rounded-lg bg-rose-500/10 border border-rose-500/20 text-rose-400 text-xs text-center font-bold uppercase tracking-wide">
                            {error}
                        </div>
                    )}
